...
use crate::{error::{ShikkaError, ShikkaResult}, poptim::Policy, strategies::sampling::{Sample, SamplingStrategy}};
...
impl<State: Copy + Pod + Zeroable, Action: Copy + Pod + Zeroable, F, T> Environment<State, Action, F, T>
where 
    F: Fn(&State) -> f32,
    T: Fn(&State, &Action) -> State
    {
    ...
    pub fn trajectory(&mut self) -> Vec<&EnvLog<State, Action>> {
        self.logs.read_all::<EnvLog<State, Action>>().into_iter().map(|x|x.0).collect::<Vec<_>>()
    }

    pub fn cleanup(&mut self) -> Vec<EnvLog<State, Action>> {
        self.logs.cleanup::<EnvLog<State, Action>>().into_iter().map(|x|x.0).collect::<Vec<_>>()
    }

    pub fn sample_forward_trajectory<P: Policy<State, Action> + 'static>(&self, horizon: usize, policy: P) -> ShikkaResult<Vec<EnvLog<State, Action>>> {
        let mut out = vec![];
        for _ in 0..horizon {
            let state = self.state()?;
            let valid = self.action_space.valid(state);
            let action = self.sampler.sample(&policy, state, valid)?;
...
unsafe impl<State: Copy + Pod + Zeroable, Action: Copy + Pod + Zeroable> Pod for EnvLog<State, Action> {}
unsafe impl<State: Copy + Pod + Zeroable, Action: Copy + Pod + Zeroable> Zeroable for EnvLog<State, Action> {}